...
//            CounterThroughputThread.instance().addJob(jobId);
    ...
    public void killJobInAgents(Collection<String> agents) throws InterruptedException, ExecutionException, IOException {
        if(!this.jobStatus.equals(JOB_STATUS.COMPLETED) &&
                !this.jobStatus.equals(JOB_STATUS.KILLED)) {

            Map<String, AgentJobStatus> agentsJobStatusMap = this.getAgentsJobStatus();
            for(String agent : agents) {
                if(!agentsJobStatusMap.get(agent).getJob_status().equals(JOB_STATUS.KILLED) &&
    ...
    public boolean isCompleted() {
        return this.getJobStatus().equals(JOB_STATUS.COMPLETED) ||
                this.getJobStatus().equals(JOB_STATUS.KILLED) ||
